...
public class PreviewUpdater : MonoBehaviour {
    ...
    IEnumerator IAnimation(float m) {
        ...
        while (f < 1f) {
            ...
            rotatingLight.transform.rotation = Quaternion.Lerp(rotatingLight.transform.rotation, Quaternion.identity, f);

            f += Time.deltaTime / duration;
            yield return null;
        }

        rotatingLight.transform.rotation = Quaternion.identity;
        EndAnimation();
    }
}
