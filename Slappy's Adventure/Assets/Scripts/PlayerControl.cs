...
public class PlayerControl : MonoBehaviour
{
    ...
    private const float INVIN_TIME = 0.5f, DEATH_BARRIER = -10f, KNOCKBACK_TIME = 1.5f, RECOVER_VEL2 = 10f;
    ...
    public float wallThreshold = 65f;
    public Vector2 wallJumpVector = new Vector2(16f, 5f);
    ...
    public GameObject catchPrevFx, swordUpgradeFx, wallJumpFx;

    [Header("Sfx")]
    public AudioClip landSound;
    public AudioClip throwSound, catchSound, swordUpgradeSound, damageSound, wallJumpSound;
    [SerializeField] private VoiceClip throwVoice, catchVoice, poundVoice, swordGetVoice, fallRespawnVoice;
    ...
    private float groundNormal = 0f; //0 is straight up
    ...
    void Update() {
        ...
        bool wallContact = landed && Mathf.Abs(groundNormal) > wallThreshold;
        ...
        if (nextState == State.none) {
            switch (state) {
                ...
                case State.pound:
                    if (landed) {
                        Debug.Log("WALL NORMAL " + groundNormal);
                        if (wallContact) {
                            //wall jump
                            bool jumpLeft = groundNormal > 0;
                            targetRotation = Quaternion.Euler(0, jumpLeft ? 179.9f : 0f, 0);
                            rigid.velocity = new Vector3(wallJumpVector.x * (jumpLeft ? -1 : 1), wallJumpVector.y);

                            if (!pounding) {
                                audios.PlayOneShot(wallJumpSound, 0.25f);
                                Fx(wallJumpFx, heldSword.transform.position, heldSword.transform.rotation);
                            }
                        }
        ...
        if (nextState != State.none) {
            ...
            switch (state) {
                case State.preThrow:
                    ...
                    throwVoice.VoiceOneShot(audios);
                    break;
                case State.thrown:
                ...
                case State.pound:
                    ...
                    CatchSword();
                    catchVoice.VoiceOneShot(audios);
                    break;
                case State.pick:
                    if (pounding) {
            ...
            case State.pound:
                if(stateTime > animTime.pound) {
                    if (!pounding) {
                        ...
                        pounding = true;
                        poundVoice.VoiceOneShot(audios);
                    }
                }
                else {
                    //lerp animator z
    ...
    private Quaternion GetAngle() {
        if (targetRotation == null) return animator.transform.rotation;
        return targetRotation.Value;
    }
    ...
    private void CheckLanded() {
        bool prevl = landed;
        Vector3 center = new Vector3(col.bounds.center.x, col.bounds.center.y - ((HEIGHT - 1f) / 2 + 0.15f), col.bounds.center.z);
        landed = Physics.OverlapSphereNonAlloc(center, 0.5f, groundCol, 1 << 6, QueryTriggerInteraction.Ignore) > 0;
        if (!prevl && landed && groundCol[0] is MeshCollider m) lastMeshEffect = m.MeshEffect();
        if(!prevl && landed) {
            audios.PlayOneShot(landSound, Mathf.Clamp01(-rigid.velocity.y / 10f) * 0.7f);
        }
    }

    private void OnCollisionEnter(Collision collision) { //one frame before landed check
        if(collision.collider.gameObject.layer == 6) {
            groundNormal = Vector2.SignedAngle(Vector2.up, collision.GetContact(0).normal);
    ...
    private void OnDrawGizmos() {
        ...
        Gizmos.DrawWireSphere(new Vector3(col.bounds.center.x, col.bounds.center.y - ((HEIGHT - 1f) / 2 + 0.15f), col.bounds.center.z), 0.5f);
        Gizmos.color = Color.cyan;
        float a = (groundNormal + 90f) * Mathf.Deg2Rad;
        Gizmos.DrawLine(transform.position, transform.position + new Vector3(Mathf.Cos(a), Mathf.Sin(a)));
    }
#endif

    private void CheckDeath() {
        Vector3 pos = PlayerPos();
        if (pos.y < DEATH_BARRIER) {
            Kill();
            fallRespawnVoice.VoiceOneShot(audios);
        }
    }
    ...
    public void FireKnockback(Vector2 knockback) {
        if (dead) return;
    ...
    public void OnPartUpdate() {
        ...
        swordGetVoice.VoiceOneShot(audios);
    }
