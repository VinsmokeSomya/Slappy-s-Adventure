...
public class MeshSeparator
{
    ...
    private static void SeparateSelectedMesh() {
        ...
        for (int i = 0; i < mesh.subMeshCount; i++) {
            ...
            CreateorUpdateMesh(sub, string.Format("{0}/{1}_s{2}.mesh", path, Selection.activeGameObject.name, i), string.Format("{0}_s{1}", Selection.activeGameObject.name, i));
        }
        AssetDatabase.SaveAssets();
    }

    private static void CreateorUpdateMesh(Mesh sub, string path, string name)
    {
