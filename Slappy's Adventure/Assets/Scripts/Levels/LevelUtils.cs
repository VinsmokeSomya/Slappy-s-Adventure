...
public static class LevelUtils {
    ...
    public static void TryUpdateRecord(float time, int levelID, RecordType type) {
        ...
        RecordType? delegrt = GetRecordTypeDelegate(type);
        if(delegrt.HasValue) TryUpdateRecord(time, levelID, (RecordType)delegrt);
    }

    public static RecordType? GetRecordTypeDelegate(RecordType type) {
        switch (type) {
            case RecordType.perfect:
                return RecordType.normal;
            case RecordType.pearlPerfect:
